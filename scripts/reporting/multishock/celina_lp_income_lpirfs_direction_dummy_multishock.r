#!/usr/bin/env Rscript
# Multi-shock version of celina_lp_income_lpirfs_direction_dummy.r
# Loops over 4 MP shock series; saves per-shock CSVs/PNGs + combined comparison outputs.
# Spec: TWFE, 4 directional interaction terms, lpirfs, Driscoll-Kraay; MAX_HORIZON=48.
...
set.seed(42)
cat("\n=== LPIRFS INCOME DIRECTIONAL DUMMIES MULTI-SHOCK ===\n\n")
...
hp_cycle <- function(x, lambda = 129600) {
  x <- as.numeric(x)
  n <- length(x)
  if (n < 4 || anyNA(x)) {
...
for (shock_var in names(SHOCKS)) {
  ...
  panel <- d_raw |>
    ...
    mutate(
      ...
      hp_cycle_income  = hp_cycle(log_income_sa, lambda = 129600),
  ...
  bench_input <- panel |>
    ...
    select(uf_code, ym_id, hp_cycle_income,
  ...
  out_csv <- file.path(OUT_TBL, sprintf("irf_income_directional_%s_twfe_dk_hp.csv", shock_var))
  ...
  p <- ggplot(plot_df, aes(x = horizon, y = estimate_1sd)) +
    ...
    labs(
      title    = sprintf("Income LP IRFs — Directional Dummies — %s (%s)", shock_label, shock_var),
      subtitle = "Outcome: HP-cycle log income (λ=129600); TWFE; Driscoll-Kraay 90% CI; 1-SD scaling",
      x        = "Horizon (months)",
      y        = "Cumulative HP-cycle log income response (1-SD directional interaction shock)"
    ) +
  ...
  out_png <- file.path(OUT_PLT, sprintf("irf_income_directional_%s_4panel_hp.png", shock_var))
  ggsave(out_png, p, width = 11, height = 8.5, dpi = 300)
  cat(sprintf("  ✓ Saved plot: %s\n", out_png))

  # Verification
  check_h    <- identical(sort(unique(shock_df$horizon)), 0:MAX_HORIZON)
  term_counts <- shock_df |> count(term) |> pull(n)
  check_terms <- all(term_counts == (MAX_HORIZON + 1))
  cat(sprintf("  Verification — horizons 0:%d: %s | full paths: %s\n",
              MAX_HORIZON,
              ifelse(check_h, "OK", "FAIL"),
...
out_csv_all <- file.path(OUT_TBL, "irf_income_directional_all_shocks_hp.csv")
...
p_comp <- ggplot(comparison_df,
  ...
  scale_x_continuous(breaks = seq(0, MAX_HORIZON, by = 6), limits = c(0, MAX_HORIZON)) +
  labs(
    title    = "Income LP IRFs — Directional Dummies — All Shock Series (TWFE)",
    subtitle = "Outcome: HP-cycle log income (λ=129600); Driscoll-Kraay 90% CI; 1-SD scaling; 4 shock series overlaid",
    x        = "Horizon (months)",
    y        = "Cumulative HP-cycle log income response (1-SD directional interaction shock)",
...
out_png_comp <- file.path(OUT_PLT, "irf_income_directional_all_shocks_comparison_hp.png")
ggsave(out_png_comp, p_comp, width = 11, height = 9, dpi = 300)
cat(sprintf("✓ Saved comparison plot: %s\n", out_png_comp))

cat("\n=== LPIRFS INCOME DIRECTIONAL DUMMIES MULTI-SHOCK COMPLETE ===\n")
