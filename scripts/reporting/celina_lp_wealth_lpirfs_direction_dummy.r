...
set.seed(42)
cat("\n=== LPIRFS WEALTH HETEROGENEITY (directional dummies, TWFE only) ===\n\n")
...
OUT_TBL <- "results/tables/lp_wealth"
OUT_PLT <- "results/plots/lp_wealth"
...
panel <- d_raw |>
  ...
  group_by(uf_code) |>
  mutate(
    wealth_proxy = net_liquid_proxy,
    asinh_wealth = asinh(wealth_proxy),
    ...
    lag1_asinh_wealth = lag(asinh_wealth),
...
CTRL_BASE <- c(
  ...
  "infl_yoy", "log_bf", "log_credit_pf", "lag1_asinh_wealth"
)

shock_terms <- c("mp_pos_x_ph2m", "mp_neg_x_ph2m", "mp_pos_x_wh2m", "mp_neg_x_wh2m")
required_cols <- c("uf_code", "ym_id", "asinh_wealth", shock_terms, CTRL_BASE)
...
out_csv <- file.path(OUT_TBL, "irf_wealth_directional_dummies_twfe_dk.csv")
...
p <- ggplot(plot_df, aes(x = horizon, y = estimate_1sd)) +
  ...
  labs(
    title = "Wealth Proxy LP IRFs with Directional Dummies (TWFE)",
    subtitle = "Outcome: asinh(net_liquid_proxy); one LP per directional interaction shock; Driscoll-Kraay 90% CI; responses scaled to 1-SD of each shock",
    x = "Horizon (months)",
    y = "Cumulative asinh wealth-proxy response (for 1-SD directional interaction shock)"
  ) +
...
out_png <- file.path(OUT_PLT, "irf_wealth_directional_dummies_twfe_dk_4panel.png")
...
cat("  - Wealth and income LPs are separated into different scripts/outputs.\n")
cat("  - Wealth outcome uses asinh(net_liquid_proxy) to handle negative values.\n")

cat("\n=== LPIRFS WEALTH HETEROGENEITY COMPLETE ===\n")
