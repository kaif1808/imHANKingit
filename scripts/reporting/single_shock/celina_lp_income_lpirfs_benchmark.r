...
set.seed(42)
cat("\n=== LPIRFS INCOME BENCHMARK (lag1_tfe + cumulative + contractionary) ===\n\n")
...
required_cols <- c(
  "uf_code", "ym_id", "log_income_sa", "mp_shock_pos",
...
extract_series <- function(res_obj, label, se_type, panel_effect_label, shock_sd) {
...
for (pe in panel_effects) {
  ...
  rows[[length(rows) + 1]] <- extract_series(res_se_ph2m, "mp_pos_x_ph2m", SE_METHOD, pe, shock_sd[["mp_pos_x_ph2m"]])
  rows[[length(rows) + 1]] <- extract_series(res_se_wh2m, "mp_pos_x_wh2m", SE_METHOD, pe, shock_sd[["mp_pos_x_wh2m"]])
}

out_df <- bind_rows(rows)
out_csv <- file.path(OUT_TBL, "irf_con_cumulative_lag1_income_lpirfs_benchmark_dk_only_dual_fe.csv")
...
p <- ggplot(plot_df, aes(x = horizon, y = estimate_1sd, linetype = panel_effect_label)) +
  ...
  labs(
    title = "Cumulative Contractionary Income LP IRFs (lpirfs benchmark)",
    subtitle = sprintf(
      "Outcome: log(mean_income_sa); baseline y[t+h]-y[t]; fixed-included log_credit_pf; response to 1-SD shock (PH2M SD=%.4f, WH2M SD=%.4f); 90%% CI",
...
out_png <- file.path(OUT_PLT, "irf_con_cumulative_lag1_income_lpirfs_benchmark_dk_only_dual_fe.png")
...
cat("\nNotes:\n")
cat("  - Outcome follows celina_lp_income.r: log(mean_income_sa).\n")
cat("  - Cumulative baseline is t: y[t+h]-y[t].\n")
cat("  - With two-way FE, common shock level is collinear with time effects; interaction terms are estimated as shocks in separate runs.\n")
...
cat("\n=== LPIRFS INCOME BENCHMARK COMPLETE ===\n")
